...
setwd("~Desktop/Info 201/m9-dplyr/exercise-7")
...
team.data <- mutate(team.data, TOVtoSTLratio = TOV/STL)

# Sort the teams from lowest turnover/steal ratio to highest
# Which team has the lowest ratio?
team.data <- arrange(team.data, TOVtoSTLratio)
...
team.data <- mutate(team.data, ASTperGAME = AST/G) %>% arrange(desc(ASTperGAME))
...
BetterShooters <- function(first.team, second.team) {
  ice.in.veins <- filter(team.data, Team %in% c(first.team, second.team)) %>%
                  filter(FG. == max(FG.)) %>%
                  select(Team, FG., PTS)
  return (ice.in.veins)
}

BetterShooters("Golden State Warriors*", "Sacramento Kings")

...
setwd("~Desktop/Info 201/m9-dplyr/exercise-4")
...
# On average, did flights gain or lose time?
  They tended to lose time.

# Create a data.frame that is of flights headed to seatac ('SEA'), 
flights.to.SEA <- flights %>% select(time.gained, dest) %>% filter(dest == 'SEA')


# On average, did flights to seatac gain or loose time?
hist(flights.to.SEA$time.gained)
